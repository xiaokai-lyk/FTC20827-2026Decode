...
public class PanBeta {
    ...
    public static class AutoPanCommand extends CommandBase {
        private final PanBeta panBeta;
        private final OdoData odoData;
        private final double fieldWidth = 365.75;

        public AutoPanCommand(PanBeta panBeta, OdoData odoData) {
            this.panBeta = panBeta;
        ...
        @Override
        public void execute() {
            double robotHeadingDeg = odoData.getHeadingDegrees();
            double robotX = odoData.getRobotX();
            double robotY = odoData.getRobotY();

            double panCurrAngle = panBeta.getCurrentAngle();
            double odoTargetAngle = Math.toDegrees(Math.atan((fieldWidth / 2 + robotY) / (fieldWidth - robotX))) + robotHeadingDeg;

            LLResult result = panBeta.limelight.getLatestResult();
            double cameraTargetAngle = panCurrAngle + result.getTx();

            if (result.isValid() && panBeta.isWithinLimits(cameraTargetAngle)) {
                panBeta.setPanAngle(cameraTargetAngle);
            } else {
                panBeta.setPanAngle(odoTargetAngle);
            }
        }

        @Override
        public void end(boolean interrupted) { panBeta.hold(); }
